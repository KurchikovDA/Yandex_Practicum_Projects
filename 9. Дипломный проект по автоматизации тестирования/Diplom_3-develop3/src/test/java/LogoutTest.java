...
public class LogoutTest extends BrowserConfiguration {
    ...
    public void logoutFromProfilePageTest() {
        ...
        mainPage.clickAccountButton(); // Нажимаем кнопку "Личный кабинет"
        profilePage.clickLogoutButton(); // Нажимаем кнопку "Выйти"
        String text = loginPage.getEnterLabelText(); // Получаем текст на кнопке входа
        Assert.assertEquals("Вход", text); // Проверяем, что текст соответствует ожидаемому
    }
